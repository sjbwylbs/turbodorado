...
public class ViewGeneratorUtil {
  ...
  public static EntityDef build(Class<?> clazz, Set<String> queryPropNameSet) {
    ...
    for (Field field : filedList) {
      ...
      if (queryPropNameSet != null && queryPropNameSet.contains(property.getName())) {
  ...
  public static void setEntitySlave(EntityDef master, EntityDef slave, String listDataPath) {
    ...
    if (listDataPath == null || listDataPath.length() == 0) {
      return;
    }
  ...
  private static Set<String> SUPPORT_DATATYPE_SET = new HashSet<String>();
  static {
    Collections.addAll(SUPPORT_DATATYPE_SET,
        new String[] {"Float", "BigDecimal", "long", "Integer", "byte",
            "Short", "Time", "boolean", "DateTime", "short", "Date", "char", "Boolean", "float",
            "String", "Calendar"});
  }
}
